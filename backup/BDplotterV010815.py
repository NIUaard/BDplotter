...
def LoadElegantSig(filename):
   conversion_t_to_z=299492458.0
   Mycommand = 'sddsprintout -noLabel -noTitle -col=s -col=Sx -col=Sxp -col=ecnx -col=s12  ' + filename + ' tmpeleganttwiss'
   os.system (Mycommand)
   sig=np.dtype({'names':['z','rms','rmsprime','emit','corr'],
               'formats':[np.double, np.double, np.double, np.double, np.double]})
   			   
   X=np.loadtxt(open("tmpeleganttwiss"), dtype=twiss)


   Mycommand = 'sddsprintout -noLabel -noTitle -col=s -col=Sy -col=Syp -col=ecny -col=s34  ' + filename + ' tmpeleganttwiss'
   os.system (Mycommand)
   sig=np.dtype({'names':['z','rms','rmsprime','emit','corr'],
               'formats':[np.double, np.double, np.double, np.double, np.double]})
   			   
   Y=np.loadtxt(open("tmpeleganttwiss"), dtype=twiss)

# 01082015: longitudinal emittance is not implemented
   Mycommand = 'sddsprintout -noLabel -noTitle -col=s -col=Ss -col=Sdelta -col=ecny -col=s56  ' + filename + ' tmpeleganttwiss'
   os.system (Mycommand)
   sig=np.dtype({'names':['z','rms','rmsprime','emit','corr'],
               'formats':[np.double, np.double, np.double, np.double, np.double]})
