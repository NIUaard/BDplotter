...
dr=np.dtype({'names':['z','gamma','K','beta','Rmax','bgrms'],'formats':[np.double,np.double,np.double,np.double,np.double,np.double,np.double,np.double]})
X=np.loadtxt(open(file24), dtype=dt)
Y=np.loadtxt(open(file25), dtype=dt)
Z=np.loadtxt(open(file26), dtype=dt)
...
plot (Z['z'],Z['rms']*0.23/360,'r-',label='\sigma_{z}')
...
# figure 2
figure()
plot (X['z'],X['emit']*1e6,label=' \gamma \epsilon_{x}')
plot (Y['z'],Y['emit']*1e6,'g--',label='\gamma \epsilon_{y}')
plot (Z['z'],Z['emit']*1e6/0.511*0.23/360,'r--',label='\gamma \epsilon_{z}')
xlabel('distance (m)', fontsize=18)
#ylabel('\sigma_{x} \mbox{~(mm)}', fontsize=18)
ylabel('2d emittances (\mu{\mbox{m)}', fontsize=18)
legend()
